...
- [x] **検証A: 弾性定数の計算** — 各ポテンシャル（Cu_zhou, Ag_u3, Au_u3）からC11, C12, C44を求め、方位別ヤング率 E[100], E[110], E[111] を算出。（simulations\Zener_ratio_calculate　にて計算）

  **結果概要**:
  各ポテンシャルにおける0Kでの弾性定数と、方位別ヤング率 $E_{hkl}$ を算出した。

  | 元素 | C11 | C12 | C44 | **Zener比 A** | **E[100]** | **E[110]** | **E[111]** |
  |:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
  | **Cu** | 180.4 | 117.1 | 70.7 | **2.232** | 88.2 | 143.4 | 181.1 |
  | **Ag** | 129.1 | 91.0 | 56.8 | **2.981** | 53.8 | 101.5 | 144.1 |
  | **Au** | 183.2 | 158.8 | 44.7 | **3.663** | 35.7 | 76.4 | 123.1 |
  (単位: GPa)

  **結論 (疑問1の解決)**:
  すべての元素で $A > 1$ であり、**$E[111] > E[110] > E[100]$** であることが確認された。
  1. **弾性域の逆転**: 引張初期に {110} の方が {100} より高い応力を示すのは、純粋に弾性異方性によるヤング率の差が原因である。
  2. **最大応力の逆転**: {110} は「硬いが脆い（活性化すべり系が少なく、低いひずみで格子不安定化する）」のに対し、{100} は「柔らかいが粘る（すべり系が多く対称性が高いため、高いひずみまで耐える）」という特性を持つ。
  3. 特に **Au は Zener比 A=3.66** と異方性が非常に強く、方位による物性の差が最も顕著に現れる。

- [x] **検証B: 一般化積層欠陥エネルギー（GSF）の計算** — {111}面を<112>方向にずらしたときのエネルギー曲線（GSF曲線）をAu, Ag, Cuについて計算し、不安定積層欠陥エネルギーγ_usを比較。　（simulations\GSF_Calculate にて計算）

  **結果概要**:
  {111}面上でのせん断障壁となるエネルギー（$\gamma_{us}$）を算出した。

  | 元素 | $\gamma_{us}$ (不安定積層欠陥エネルギー) | $\gamma_{isf}$ (固有積層欠陥エネルギー) |
  |:---:|:---:|:---:|
  | **Cu** | **194.4** | 85.9 |
  | **Ag** | **118.7** | 1.5 |
  | **Au** | **95.2** | 4.7 |
  ...
  **結論 (疑問2の解決)**:
  1. **{111}強度の支配因子**: {111}方向の理想強度は、ヤング率よりも「滑り」の物理的障壁である $\gamma_{us}$ に強く相関することが確認された（Cu >> Ag > Au の順序が一致）。
  2. **AuとAgの類似性**: {100}や{110}方位では弾性率の差（検証A）により Au < Ag と明確な差が出るが、{111}方位では滑り障壁である $\gamma_{us}$ が Au と Ag で比較的近い値（Cuとの差に比べて小さい）であるため、最大応力がほぼ同等になったと考えられる。
  3. **Cuの突出した強度**: Cuは全元素中で $\gamma_{us}$ が圧倒的に高く、これが全方位において高い耐力を示す根本的な要因である。
